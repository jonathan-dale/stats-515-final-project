...
# ## Sanity check
# sum(is.na(model_df$BMI))
# sum(is.na(model_df$age))
# sum(is.na(model_df$gender))
# sum(is.na(model_df$days_physical))
# sum(is.na(model_df$smoking))
...
fit_age = lm(BMI ~ age, data = model_df)
summary(fit_age)
par(mfrow = c(2, 2))
plot(fit_age)

fit_physical = lm(BMI ~ days_physical, data = model_df)
plot(fit_physical)
